...
func (m *model) headerView() string {
	...
	if n := len(m.errs); n > 0 {
		...
		right = m.theme.okStyle().Render(fmt.Sprintf("copied %dB  c", m.copiedN))
	}
...
func (m *model) footerText() string {
	return " / filter   n ns   ←→ scroll   j/k move   c copy   f follow   t time   p pause   d detail   s context   e errors   ? help   q quit"
}
